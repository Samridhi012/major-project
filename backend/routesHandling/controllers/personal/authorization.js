...
export const registerUser = async (req, res) => {
  ...
  const saltRounds = 10; // Define the complexity for hashing
  ...
  try {
    ...
    if (existingUser.rows.length > 0) {
     ...
     // 2. Hash the plaintext password before storing
    ...
    // 3. Store the HASHED password (note the column change from 'password' to 'password_hash')
...
export const loginUser = async (req, res) => {
  ...
  try {
    // NOTE: We select the password_hash column now
    ...
    // 4. Use bcrypt.compare() to check the plaintext password against the stored hash
    // The column name is now 'password_hash' (assuming your table is updated)
